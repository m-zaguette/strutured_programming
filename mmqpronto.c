...
#include<stdlib.h>
#include<math.h>
void valoresxy(float *px,float *py,int tot);//Usuário insere os valores de x e de y;
void mmqmatriz2(float *x, float *pu0, float *pu1, int tot);//Criará a matriz utilizada para Mínimos Quadrados com grau 1 (o número 2 corresponde a 
//quantidade de variáveis;
void mmqmatriz3(float *x, float *pu0, float *pu1, float *pu2, int tot);//Criará a matriz utilizada para Mínimos Quadrados com grau 2 (o número 3 corresponde a 
//quantidade de variáveis;
//void preenchermatriz(float *p, int lin, int col); //Essa funcao era utilizada para o usuario inserir os valores da matriz, porém como já
//é calculado o valor através dos mínimos quadrados o usuário não inserirá valores;
void preenchermatriz2 (float *pu0, float *pu1, float *py, float *p, int tot, int col);//Funcao feita para transferir e calcular os valores corretos dos Mínimos
//quadrados para a matriz inicial 3x3
void preenchermatriz3(float *pu0, float *pu1, float *pu2, float *py, float *p, int tot, int col);//Funcao feita para transferir e calcular os valores corretos dos Mínimos
//quadrados para a matriz inicial 4x4
void imprimir (float *p, int lin, int col);//Funcao imprime
void gauss3 (float *p, int lin, int col);//Resolucao de Gauss com duas variáveis através da utilização de pivos e seus valores correspondentes;
void gauss2 (float *p, int lin, int col);//Resolucao de Gauss com duas variáveis através da utilização de pivos e seus valores correspondentes;
void resolve2(float *p, int lin, int col);//Resolucao do sistema linear com 2 variáveis e exibição do polinômio correspondente;
void resolve3(float *p, int lin, int col);//Resolucao do sistema linear com 3 variáveis e exibição do polinômio correspondente;
...
int main(){
	...
	do{
		...
		printf("Deseja calcular novamente com outro grau? <1/0>: ");
		scanf("%i",&parartotal);
	}while(parartotal!=0);
	return 0;
}
...
void gauss3 (float *p, int lin, int col){
	...
	//printf("\n");
	//printf("\nO primeiro pivo valor: %.4f",pivo1);
	//printf("\nO segundo pivo valor: %.4f",pivo2);
	//printf("\nO terceiro pivo valor: %.4f",pivo3);
	int i=0,j=0;
	pivo1 *= -1;
	pivo2 *= -1;
	for(i=0;i<col;i++){
...
void resolve2(float *p, int lin, int col){
	...
	printf("\nValor na primeira linha(x): %.4f",linha1);
	printf("\nValor na segunda linha(y): %.4f\n",linha2);
	printf("\n\tp(x) = (%.4f) + (%.4f)*x\n",linha1,linha2);
}
...
void resolve3(float *p, int lin, int col){
	...
	printf("\nValor na primeira linha(x): %.4f",linha1);
	printf("\nValor na segunda linha(y): %.4f",linha2);
	printf("\nValor na terceira linha(z): %.4f\n",linha3);
